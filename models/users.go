...
type UserDB interface {
	...
	ByUserID(userID uint) ([]User, error)
...
func (uv *userValidator) Delete(id uint) error {
	var user User
	user.ID = id
	err := runUserValFuncs(&user, uv.idGreaterThan(1))
	if err != nil {
		return err
	}
...
func (ug *userGorm) ByUserID(userID uint) ([]User, error) {
	var users []User
	var err error

	if userID == 1 {
		err = ug.db.Order("id asc").Find(&users).Error
	} else {
